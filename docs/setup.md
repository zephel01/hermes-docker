# hermes-docker — セットアップ & 運用手順

`nousresearch/hermes-agent` + WebUI + HUD + SearXNG を docker compose で
**ゼロから組み立てて健全に運用する**ための詳細手順書。

障害発生時の調査記録は [`TROUBLESHOOTING.md`](./TROUBLESHOOTING.md) を参照。
リポジトリ全体の概要・Quick start は [`../README.md`](../README.md) を参照。
...
3. [初回構築手順](#3-初回構築手順)
4. [健全性確認](#4-健全性確認)
5. [コンテナ内 hermes CLI による初期セットアップ](#5-コンテナ内-hermes-cli-による初期セットアップ)
6. [既知のローカルパッチ（webui xai-oauth）](#6-既知のローカルパッチwebui-xai-oauth)
7. [日常運用コマンド](#7-日常運用コマンド)
8. [アンインストール](#8-アンインストール)
9. [このスタックの「設計上わかっておくと事故が減る」点](#9-このスタックの設計上わかっておくと事故が減る点)
...
> [TROUBLESHOOTING §3](./TROUBLESHOOTING.md#3-根本原因-環境変数の-split-brain検索が接続できない) を参照。
...
├── .gitignore
├── .gitattributes                 # *.sh / *.patch を eol=lf に固定 (WSL2 対応)
├── .gitmodules                    # hermes-webui / hermes-hudui の URL と pinned commit を記録
├── docker-compose.yml             # 4 サービス定義 + named volume + network
├── README.md                      # ランディング (概要 / 図 / Quick start / docs リンク)
│
├── docs/
│   ├── setup.md                   # この手順書
│   └── TROUBLESHOOTING.md         # 既知の不具合・復旧手順・compose 設計上の弱点
│
...
壊れる**（[TROUBLESHOOTING §4](./TROUBLESHOOTING.md#4-根本原因-xai-oauth-のモデル一覧が空grok-43-が表示されない) 直結）。詳細は §6 参照。
...
**悪いログの兆候**（出ていたらすぐ [TROUBLESHOOTING §9](./TROUBLESHOOTING.md#9-根本原因-hermes-agent-src-ボリュームの陳腐化restarting-127-ループ) へ）:
...
[TROUBLESHOOTING §4](./TROUBLESHOOTING.md#4-根本原因-xai-oauth-のモデル一覧が空grok-43-が表示されない) を参照**。
...
3. **CLI 実体のパスは `/opt/hermes/.venv/bin/hermes`**。`/opt/hermes` は named volume
   ...
   このパスごと消えることがある（[TROUBLESHOOTING §9](./TROUBLESHOOTING.md#9-根本原因-hermes-agent-src-ボリュームの陳腐化restarting-127-ループ)）。
...
> webui エージェントから見えず壊れる（[TROUBLESHOOTING §3](./TROUBLESHOOTING.md#3-根本原因-環境変数の-split-brain検索が接続できない)）。
...
- **`docker compose pull` 後に `hermes` コマンドが not found** → named volume の
  staleness。[TROUBLESHOOTING §9](./TROUBLESHOOTING.md#9-根本原因-hermes-agent-src-ボリュームの陳腐化restarting-127-ループ) の手順で復旧。
...
という上流バグへの対処（[TROUBLESHOOTING §4](./TROUBLESHOOTING.md#4-根本原因-xai-oauth-のモデル一覧が空grok-43-が表示されない) で発生時の症状確認可能）。
...
出ていたら named volume の staleness（[TROUBLESHOOTING §9](./TROUBLESHOOTING.md#9-根本原因-hermes-agent-src-ボリュームの陳腐化restarting-127-ループ)）。
復旧は次:
...
[TROUBLESHOOTING §10](./TROUBLESHOOTING.md#10-docker-composeyml-の既知の設計上の弱点) に詳しいが要点だけ:
